    ...
    do {                                                                       \
        ...
        if (_errno != 0)                                                       \
            fprintf(stderr,                                                    \
                    ...
                    "%d.\n\tErrno: (%d; %s)\n",                                \
                    #expr, __func__, __FILE__, __LINE__, _errno,               \
                    strerror(_errno));                                         \
    } while (0)
...
static void delay(const char *delay_var, const size_t size) {
    ...
    if (delay_str) {
        ...
        if (delay_ms > 0) {
            msleep((size + ATOMIC_BLOCK_SIZE - 1) / ATOMIC_BLOCK_SIZE *
                   delay_ms);
        }
    }
...
int channel(int pipefd[2]) { return pipe(pipefd); }
...
void channels_finalize() { ASSERT_ZERO(pthread_mutex_destroy(&mutex)); }
